...
- [x] **Role-based UI guards** — re-audited: Users, Assets, Content/CreateContent/ViewContent, Notification Settings, and Personalization Settings were already correctly wrapped in `<AuthorizeView>` in their `.razor` files — their `.razor.cs` TODOs (and the `ImageGalleryDialog`/`AuthorSelect` caveats, which are only ever used inside the already-guarded Content pages) were stale and have been removed. Two real gaps remained and are now fixed. Next applies RBAC consistently per-route via `rbac.ts`.
    - **Comments Settings** — had no `<AuthorizeView>`; `Load` hits `SettingsClient.GetOwnerData` (`ROLE_IS_OWNER_OR_SERVICE`), saves use `ModifyComments*Public/Private` (`admin,owner`) or `...Owner` (`owner` only). Wrapped the whole page in `<AuthorizeView Roles="@RoleAbilities.ROLE_IS_OWNER_OR_SERVICE">` since that's the strictest requirement (the initial load) — matches the existing pattern in `NotificationSettings.razor`/`PersonalizationSettings.razor`.
        - Blazor: `Components/Pages/Settings/Comments/CommentsSettings.razor(.cs)`
    - **Dashboard (Home)** — had no `<AuthorizeView>`; `GetKpis` requires `admin,owner,service,bot_verification`. Wrapped the whole page in `<AuthorizeView Roles="@RoleAbilities.ROLE_IS_ADMIN_OR_OWNER_OR_SERVICE_OR_BOT">`.
        - Blazor: `Components/Pages/Home.razor`
